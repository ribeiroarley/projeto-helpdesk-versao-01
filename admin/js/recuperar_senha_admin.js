...
// Adiciona um evento de submit ao formulário de recuperação de senha do admin
document.getElementById("recuperarSenhaAdminForm").addEventListener("submit", function(event) {
    ...
    // Verifica se o e-mail existe no cadastro de administradores
    const admins = JSON.parse(localStorage.getItem("admins")) || [];
    const adminExists = admins.some(admin => admin.email === email);
    if (!adminExists) {
        showMessage("Este e-mail não está cadastrado como administrador.", "error");
        return;
    }
